  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
            ...
            itemBuilder: (context, index) {
              ...
              return AnimatedBuilder(
                ...
                builder: (context, child) {
                  ...
                  final scale = 1.0 - (distance * 0.16);
                  final opacity = (1.0 - (distance * 0.55)).clamp(0.45, 1.0);
                  return Center(child: Opacity(opacity: opacity, child: Transform.scale(scale: scale, child: child)));
                },
        ...
        AnimatedSwitcher(duration: const Duration(milliseconds: 350), transitionBuilder: (child, animation) => FadeTransition(opacity: animation, child: child), child: Text(current.name, key: ValueKey('name_${current.id}'), style: theme.textTheme.headlineMedium, textAlign: TextAlign.center)),
        const SizedBox(height: 4),
        AnimatedSwitcher(duration: const Duration(milliseconds: 300), transitionBuilder: (child, animation) => FadeTransition(opacity: animation, child: child), child: Text('€${current.basePrice.toStringAsFixed(2)}', key: ValueKey('price_${current.id}'), style: theme.textTheme.titleLarge?.copyWith(color: theme.colorScheme.primary))),
        const SizedBox(height: 14),
        if (widget.products.length > 1) _PageDots(count: widget.products.length, activeIndex: activeIndex),
      ],
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.12), blurRadius: 24, offset: const Offset(0, 14))],
      ),
      child: Hero(tag: 'product_image_${product.id}', child: ProductImage(imageUrl: product.imageUrl, borderRadius: 28)),
    );
  }
}
    ...
    return Row(
      ...
      children: List.generate(count, (index) {
        ...
        return AnimatedContainer(
          ...
          width: isActive ? 20 : 7,
          height: 7,
          decoration: BoxDecoration(color: isActive ? Colors.black87 : Colors.black26, borderRadius: BorderRadius.circular(4)),
        );
      }),
    );
  }
