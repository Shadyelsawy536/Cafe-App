...
class OrderingController extends ChangeNotifier {
  ...
  Future<void> loadData() async {
    ...
    try {
      // Critical data required for the initial Home screen.
      ...
      // The Home screen can render now.
      loading = false;

      notifyListeners();

      // Non-critical data loads in the background.
      unawaited(
        _repository.fetchLocations().then((freshLocations) {
          if (_disposed) return;
          ...
          debugPrint(
            'Background locations load error: $e',
          );
        }),
      );
