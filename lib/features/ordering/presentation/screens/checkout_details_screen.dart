...
class CheckoutDetailsScreen
  ...
  @override
  State<CheckoutDetailsScreen>
      createState() =>
          _CheckoutDetailsScreenState();
}

class _CheckoutDetailsScreenState
    extends State<CheckoutDetailsScreen> {
  final _formKey =
      GlobalKey<FormState>();
  ...
  void initState() {
    ...
    _deliveryType =
        previous?.deliveryType ??
            DeliveryType.delivery;

    _pickupBranch =
        previous?.pickupBranch ??
                    ...
                    OutlinedButton.icon(
                      ...
                      label: Text(
                        _scheduledFor ==
                                null
                            ? 'Order now'
                            : 'Scheduled: ${_formatDateTime(_scheduledFor!)}',
                      ),
                      onPressed: () =>
                          _pickSchedule(
                        settings,
  ...
  Future<void> _pickSchedule(
    ...
    if (selected.isBefore(
      ...
      ScaffoldMessenger.of(
        ...
        const SnackBar(
          content: Text(
            'Please choose a time after the preparation window.',
          ),
        ),
      );
      return;
  ...
  Future<void> _submit() async {
    ...
    if (totals.subtotal <
      ...
      ScaffoldMessenger.of(
        ...
        SnackBar(
          content: Text(
            'Minimum order is ${CurrencyFormatter.format(settings.minOrderAmount, settings.currency)}',
          ),
        ),
      );
      return;
    ...
    if (orderId != null) {
      try {
        ...
        ScaffoldMessenger.of(
          ...
          SnackBar(
            content: Text(
              'Order placed, but order details could not be saved: $e',
            ),
          ),
        );
      }
  ...
  ActionButtonStatus _mapCheckout(
