...
abstract class ProductRepository {
  /// Loads the lightweight catalog used by the home/catalog screens.
  ///
  /// This does NOT load product variants or modifier configuration.
  Future<List<Product>> fetchProducts();

  /// Returns the cached complete configuration for a product.
  ///
  /// Returns null when the product configuration has not been cached yet.
  Product? getCachedProductConfiguration(
    String productId,
  );

  /// Loads the complete configuration for one product.
  ///
  /// This includes sizes/variants and modifier groups.
  Future<Product> fetchProductConfiguration(
    String productId,
  );

  /// Prefetches product configurations in the background.
  ///
  /// Already cached products and products with an active request
  /// should not trigger another network request.
  Future<void> prefetchProductConfigurations(
    Iterable<String> productIds,
  );

  /// Invalidates the cached configuration for one product.
  void invalidateProductConfiguration(
    String productId,
  );

  /// Invalidates all cached product configurations
  /// that use this modifier group.
...
class MockProductRepository implements ProductRepository {
  ...
  Future<List<CafeLocation>> fetchLocations() async {
    ...
    return const [
      CafeLocation(
        name: 'Central Avenue',
        address: 'First Sheikh Zayed, Albostan St.',
      ),
      CafeLocation(
        name: 'Fuel Up Pyramids View',
        address: 'Fuel Up gas station, Ring Road',
      ),
    ];
  }
  ...
  Future<List<Product>> fetchProducts() async {
    ...
    return [
      ...
      const Product(
        ...
        name: 'Matcha Latte',
        description:
            'Creamy Japanese matcha whisked with steamed milk.',
        basePrice: 3.8,
        imageUrl:
